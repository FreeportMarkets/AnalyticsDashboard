...
export async function JourneyDaily({ from, to }: { from?: string; to?: string } = {}) {
  ...
  return AccountCohortsSection({ from: from ?? (validEnd ? addDays(end, -29) : end), to: end, initialMetric: 'firstTrade' })
}
